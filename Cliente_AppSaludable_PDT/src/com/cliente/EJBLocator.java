...
/**
* Obtiene el JNDI para el bean especificado en el modulo
*/
...
/**
* Obtiene el JNDI para el bean especificado en el modulo específico.
*/
...
public <T> String getJNDIName(final Class<T> ejbClass, final String modulo) {
	...
	if (ejbClass.getSimpleName().endsWith("Local")) {
		jndiName += ejbClass.getSimpleName().substring(0,
				ejbClass.getSimpleName().lastIndexOf("Local"));
	} else if (ejbClass.getSimpleName().endsWith("Remote")) {
		jndiName += ejbClass.getSimpleName().substring(0,
		ejbClass.getSimpleName().lastIndexOf("Remote"));
	}
...
/**
* Devuelve la referencia al EJB que implementa la interfaz remota o local que se le
pasa.
